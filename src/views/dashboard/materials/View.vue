<template>
    <div class="content">
        ...
        <router-link :to="`/dashboard/materials/edit/${$route.params.id}`" tag="button" class="btn btn-secondary float-right">
            Materiaal aanpassen
    ...
    methods: {
            ...
            try {
                await this.$store.dispatch("alert", {
                    ...
                    msg: {
                        ...
                        text: "Weet je zeker dat je dit materiaal wilt verwijderen? Wanneer je dit materiaal verwijderd verlies je al de data en dit kan niet meer ongedaan worden gemaakt."
                    }
                });
                this.deleteMaterial();
            } catch (error) {
        ...
        syncMaterial() {
            if (this.materials) {
                ...
                const currentMaterial = this.materials.filter((item) => item.id === id)[0];
