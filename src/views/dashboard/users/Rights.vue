                ...
                <div class="card mt-5">
                    ...
                    <div class="card-body">
                        <div class="form-group mb-0">
                            ...
                            <select class="selectpicker form-control" data-live-search="true" data-style="btn-neutral border" v-model="selectedEditorID">
                                <option selected disabled hidden>Choose...</option>
                                <option v-for="user of users" :key="user.id" :value="user.id">{{ user.firstName }} {{ user.lastName }}</option>
                            </select>
                        </div>
                        <button class="btn btn-default d-block mt-0 ml-auto" @click="addRole('editor')">Maak beheerder</button>
            ...
            <div class="col-sm-6">
                <div class="card mt-5">
                    ...
                    <div class="card-body">
                        <div class="form-group mb-0">
                            <label>Kies gebruiker</label>
                            <select class="selectpicker form-control" data-live-search="true" data-style="btn-neutral border" v-model="selectedAdminID">
                                <option v-for="user of users" :key="user.id" :value="user.id">{{ user.firstName }} {{ user.lastName }}</option>
                            </select>
                        </div>
...
export default {
    ...
    computed: {
        isLoaded() {
            if (this.users.length && this.editors.length && this.admins.length) {
        ...
        addRole(type) {
            ...
            if (editor || admin) {
                this.$store.dispatch("addRole", {
                    type,
                    id: type === "editor" ? this.selectedEditorID : this.selectedAdminID
                });
            }
        },
        removeRole(type, id) {
