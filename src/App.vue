<template>
    <div id="app">
        <!-- FRONDEND -->
        <!-- front Header -->
        <transition enter-active-class="animated slideInTop" leave-active-class="animated slideOutUp" mode="out-in">
            <NavbarFront v-if="front" style="animation-duration: 0.5s" />
        </transition>
        <!--  Front Router View-->
        <transition :name="transitionEnter" :leave-active-class="transitionLeave" mode="out-in">
            <router-view style="animation-duration: 0.5s" v-if="front || landing" />
        </transition>
        <!-- Front Footer  -->
        ...
        <!-- BACKEND -->
        <!-- back Sidebar -->
        <transition name="slideLeft" leave-active-class="animated slideOutLeft" mode="out-in">
            <SidebarBack style="animation-duration: 0.8s" v-if="back" />
        </transition>
        <div class="main-panel h-100">
            <!-- back Header -->
            <transition name="slideDown" leave-active-class="animated slideOutUp" mode="out-in">
            ...
            <!-- back Footer-->
            <transition name="slideUp" leave-active-class="animated slideOutDown" mode="out-in">
